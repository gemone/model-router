...
- **规则引擎** - 基于条件的动态路由规则，支持按模型名称、请求内容、时间等条件匹配
- **复合模型** - 支持并行/串行调用多个模型，结果聚合与综合回答
- **消息压缩** - 智能上下文压缩，支持滑动窗口、语义聚类、级联等策略
- **登录认证** - JWT 认证系统，保护管理界面安全
...
支持并行/串行调用多个模型，实现结果聚合：

- **并行策略** - 同时调用多个模型，提高响应速度
- **串行策略** - 顺序调用多个模型，实现链式处理
- **聚合方法** - 投票、平均、最佳答案选择等
